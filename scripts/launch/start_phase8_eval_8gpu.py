...
def main():
 ...
 for arm in ['S10','K01','K02']:
  if (a/('eval-'+arm)/'DONE.json').exists():
   status('reuse_completed_evaluation',arm=arm);continue
  status('eight_gpu_serial_evaluation',arm=arm,gpus=list(range(8)))
  wait({arm:launch('eightgpu-eval-'+arm,'scripts/eval/eval_phase8_sequence_8gpu.py',['--arm',arm],'0,1,2,3,4,5,6,7')})
 status('comparison')
 import numpy as np
