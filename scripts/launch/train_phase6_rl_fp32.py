...
def prepare_old(model,reference,tok,episodes,audit=False):
    ...
    # FP32 cached generation and teacher-forced forward need not be bitwise identical.
...
def main():
    ...
    torch.set_num_threads(4);torch.backends.cuda.matmul.allow_tf32=False;torch.backends.cudnn.allow_tf32=False;torch.manual_seed(c["seed"]);torch.cuda.set_per_process_memory_fraction(.15)
    ...
    sourcefiles=["scripts/launch/train_phase6_rl_fp32.py","scripts/phase6_agent_common.py","scripts/phase6_agent_v2.py","scripts/phase6_retention_env.py",
            ...
            for inner in range(c["inner_updates"]):
                ...
                for i in range(0,len(actions),2):
                    ...
                    for action,new in zip(chunk,current):
                        ...
                        # Small FP32 actor/generator discrepancy corrected as a detached token IS factor.
