...
from train_phase6_rl import strict_call,sha,write
...
MODELS={"A01":ART/"A01-v3-mixed-lr3e6-20260908/checkpoint-570",
        "A02":ART/"A02-agentic-grpo-v1-20260908/model-fp32",
        "A03":ART/"A03-agentic-cispo-v1-20260908/model-fp32"}
def main():
 ...
 modelpath=MODELS[a.arm];out=ART/f"{a.arm}-agentic-rl-v1-{a.split}-20260908"
 assert not out.exists()
 if a.split=="test":
  freeze=json.loads(Path("experiments/05-agentic-rl/rl-v1-test-freeze.json").read_text())
 ...
 out.mkdir()
 torch.manual_seed(42);torch.set_num_threads(4);torch.cuda.set_per_process_memory_fraction(.15)
 ...
 import swanlab
 run=swanlab.init(project="MiniMind-Lab",experiment_name=f"Phase6-{a.arm}-AgentRL-v1-{a.split}",
 ...
 for domain,mod,file in (("graph","phase6_agent_v2",a.split+".jsonl"),("tools","phase6_retention_env","tools-"+a.split+".jsonl")):
  ...
  summary=evaluation.evaluate(model,tok,tasks,out/domain)
 ...
 write(out/"eval-manifest.json",{"arm":a.arm,"split":a.split,"model":str(modelpath),"model_sha256":sha(modelpath/"model.safetensors"),
       ...
       "source":{str(p):{"sha256":sha(p),"text":p.read_text()} for p in [Path(__file__),Path("scripts/eval/eval_phase6_agent.py"),Path("scripts/launch/train_phase6_rl.py")]},
       "legacy_behavior_dtype":"fp16","agent_dtype":"fp32+bf16-autocast"})
