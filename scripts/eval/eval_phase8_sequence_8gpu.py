...
from phase8_eight_gpu import EightGPU
...
def main():
 ...
 gpu='0,1,2,3,4,5,6,7';assert os.environ['CUDA_VISIBLE_DEVICES']==gpu
 used=subprocess.check_output(['nvidia-smi','--query-gpu=memory.used','--format=csv,noheader,nounits'],text=True).split()
 assert len(used)==8 and all(int(x)<512 for x in used),('GPU not free',used)
 ...
 torch.set_num_threads(4);torch.manual_seed(42);torch.set_num_interop_threads(2)
 model=MiniMindForCausalLM(MiniMindConfig());model.load_state_dict(torch.load(ck,map_location='cpu',weights_only=True),strict=True);model=model.half().cpu().eval()
 tok=AutoTokenizer.from_pretrained(ROOT/'minimind/model',local_files_only=True)
 lm=EightGPU(pretrained=model,tokenizer=tok,backend='causal',batch_size=1,max_length=32768,add_bos_token=False)
 ...
 manifest['parallelism']='8 independent GPU replicas; request round-robin; order restored; per GPU generation batch1 / likelihood batch16'
 dump(out/'manifest.json',manifest)
 lm.start_pool(ck,out)
 ...
 if all((cache/n).exists() for n in required):
  ...
  raise RuntimeError('8GPU recovery requires verified saved behavior results')
