#!/usr/bin/env bash
set -Eeuo pipefail
EXPERIMENT_ID=${EXPERIMENT_ID:-L01-code-full-ft-20260907}
...
torchrun --standalone --nproc_per_node=6 train_sft_with_validation.py \
  --save_dir "$ARTIFACT_DIR/checkpoints" --save_weight l01_last --best_weight l01_best_val \
  --epochs 3 --batch_size 16 --learning_rate 1e-5 --dtype bfloat16 \
  ...
  --split_seed 42 --split_manifest "$ARTIFACT_DIR/split/manifest.json" \
  ...
  --swanlab_run_name L01-Code-FullFT-S10-64M --use_compile 0 \
